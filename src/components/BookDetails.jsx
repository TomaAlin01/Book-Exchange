...
const BookDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      try {
        const bookRef = ref(db, `books2/${id}`);
        const snapshot = await get(bookRef);

        if (snapshot.exists()) {
          const bookData = snapshot.val();
          console.log('BookDetails - Firebase Book Data:', bookData);
          setBook(bookData);
        } else {
...
<header>
  ...
  <nav>
    ...
    <ul>
      <li><a href="http://localhost:3000/home#top">Home</a></li>
      <li><a href="http://localhost:3000/booklist">Book List</a></li>
      <li>
        {!isLoggedIn ? (
...
<button type="submit">Sign Up</button>
      ...
      <main>
        <section className="book-details">
          <img src="../book_general.jpg" alt="Book 1" />
          <h2>{book.title}</h2>
